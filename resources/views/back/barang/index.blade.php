                                        ...
                                        <table id="responsive-datatable" class="table table-bordered table-bordered dt-responsive nowrap">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{$item->jenis}}</td>
                                                    <td>
                                                        @if ($item->peminjaman && ($item->peminjaman->status == 'dikembalikan' || $item->peminjaman->status == 'ditolak'))
                                                            Tersedia
                                                        @else
                                                            {{ $item->peminjaman ? ($item->peminjaman->status ?? 'dikembalikan') : 'Tersedia' }}
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td class="text-center">                                                 
                                                        ...
                                                        @if (auth()->user()->level == 'pengguna' && (($item->peminjaman->status ?? $item->status) == 'dikembalikan' || ($item->peminjaman->status ?? $item->status) == 'ditolak'))
...
<script>
    function pinjamBarang(kodeBarang) {
        $.ajax({
            ...
            type: 'POST',
            url: '{{ route("barang.pinjam") }}', // Ubah URL sesuai dengan rute untuk metode pinjam di BarangController
            data: {
                kode_barang: kodeBarang
            },
            dataType: 'json',
            success: function (response) {
                alert('Barang berhasil dipinjam: ' + response.message);
                // Di sini Anda dapat melakukan tindakan tambahan, jika diperlukan
            },
            error: function (xhr, ajaxOptions, thrownError) {
                alert('Gagal meminjam barang: ' + xhr.responseText);
            }
        });
    }
</script>
